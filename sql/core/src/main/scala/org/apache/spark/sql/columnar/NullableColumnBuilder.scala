...
import org.apache.spark.sql.catalyst.types.BinaryType.JvmType
...
private[sql] trait NullableColumnBuilder extends ColumnBuilder {
  ...
  abstract override def appendFrom(row: Row, ordinal: Int): Unit = {
    columnStats.gatherStats(row, ordinal)  //更新statistics信息
    if (row.isNullAt(ordinal)) {
      nulls = ColumnBuilder.ensureFreeSpace(nulls, 4)
      nulls.putInt(pos)
      nullCount += 1
    } else {
      super.appendFrom(row, ordinal)  //BasicColumnBuilder写入buffer中
    }
    pos += 1
  }
