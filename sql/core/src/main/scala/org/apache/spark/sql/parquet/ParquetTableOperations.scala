    ...
    if (partOutput.nonEmpty) {
      baseRDD.mapPartitionsWithInputSplit { case (split, iter) =>
        ...
        new Iterator[Row] {
          ...
          def next() = {
            //zengdan
            val joinedRow2 = new JoinedRow5(Row(partitionRowValues:_*), null)
            val result = joinedRow2.withRight(iter.next()._2)
            //zengdan
            //joinedRow.withRight(iter.next()._2)
            result
          }
        }
      }
...
/*
//@zengdan
private[parquet] class CachedBatchRecordReaderImpl
  extends parquet.io.RecordReaderImplementation[Row]() with Logging{
  ...
  override def read(): Row = {
    ...
    var currentState: RecordReaderImplementation.State = states(0)


    do {
      val columnReader: ColumnReader = currentState.column
      val d: Int = columnReader.getCurrentDefinitionLevel
      val depth: Int = currentState.definitionLevelToDepth(d)
      ...
      val nextR: Int = if (currentState.maxRepetitionLevel == 0) 0 else columnReader.getCurrentRepetitionLevel
      val next: Int = currentState.nextLevel(nextR)
