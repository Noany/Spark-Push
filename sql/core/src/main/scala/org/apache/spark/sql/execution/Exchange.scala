    ...
    newPartitioning match {
      case HashPartitioning(expressions, numPartitions) =>
        ...
        val rdd = if (sortBasedShuffleOn) {
          //zengdan
          @transient lazy val hashExpressions = newProjection(expressions, child.output)
          child.execute().map { r => (hashExpressions(r), r.copy())}
          ...
          */
          ...
          //zengdan
          @transient lazy val hashExpressions = newMutableProjection(expressions, child.output)()
          @transient lazy val mutablePair = new MutablePair[Row, Row]()
          child.execute().map { r => mutablePair.update(hashExpressions(r), r)}
