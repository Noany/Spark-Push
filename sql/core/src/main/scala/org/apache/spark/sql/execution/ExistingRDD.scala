  ...
  def productToRowRdd[A <: Product](data: RDD[A]): RDD[Row] = {
    ...
    data.map{x =>
      if (x == null) {
        ...
        //val size = bufferedIterator.head.productArity
        val mutableRow = new GenericMutableRow(x.productArity)

        var i = 0
        while (i < mutableRow.length) {
           mutableRow(i) = ScalaReflection.convertToCatalyst(x.productElement(i))
           i += 1
        }
    ...
    data.mapPartitions { iterator =>
      if (iterator.isEmpty) {
        ...
        bufferedIterator.map { r =>
          //val mutableRow = new GenericMutableRow(size) //zengdan
          var i = 0
          while (i < mutableRow.length) {
