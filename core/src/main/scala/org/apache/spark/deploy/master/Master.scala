...
private[spark] class Master(
  ...
  override def preStart() {
    ...
    persistenceEngine = RECOVERY_MODE match {//zengdan 存放（app,driver,worker）信息
      case "ZOOKEEPER" =>
    ...
    leaderElectionAgent = RECOVERY_MODE match {//zengdan 选择leader
        case "ZOOKEEPER" =>
  ...
  private def schedule() {
    ...
    for (driver <- waitingDrivers.toList) { // iterate over a copy of waitingDrivers
      ...
      log.debug("worker size when launching driver is %d".format(workers.size))
      log.debug("worker size after filter when launching driver is %d".format(shuffledAliveWorkers.size))
      var launched = false
    ...
    if (spreadOutApps) {
      ...
      for (app <- waitingApps if app.coresLeft > 0) {
        ...
        log.debug("worker size when launching executor is %d".format(workers.size))
        log.debug("worker size after filter state when launching executor is %d".format(workers.toArray.
          filter(_.state == WorkerState.ALIVE).size))
        log.debug("worker size after chech usable when launching executor is %d".format(usableWorkers.size))
