...
private[spark] class CacheManager(blockManager: BlockManager) extends Logging {
  ...
  //zengdan
  def get[T](rdd: RDD[T],partition: Partition, context: TaskContext, storageLevel: StorageLevel):
    Option[Iterator[T]] = {
    ...
    blockManager.get(key) match {
      case Some(blockResult) =>
        ...
        Some(new InterruptibleIterator(context, blockResult.data.asInstanceOf[Iterator[T]]))

      case None =>
        waitLockForPartition(key)
        //acquireLockForPartition(key)
    }
  }
  ...
  def put[T](
    ...
    blockManager.get(key) match {
      ...
      case None =>
        ...
        if (storedValues.isDefined) {  //others have already finished puting it in block
        ...
        try {
          ...
          val cachedValues = putInBlockManager(key, data.toIterator, storageLevel, updatedBlocks)
    ...
    blockManager.get(key) match {
      ...
      case None =>
        ...
        try {
          ...
          // Otherwise, cache the values and keep track of any updates in block statuses
          val updatedBlocks = new ArrayBuffer[(BlockId, BlockStatus)]
          val cachedValues = putInBlockManager(key, data, storageLevel, updatedBlocks)
          val metrics = context.taskMetrics
          val lastUpdatedBlocks = metrics.updatedBlocks.getOrElse(Seq[(BlockId, BlockStatus)]())
          metrics.updatedBlocks = Some(lastUpdatedBlocks ++ updatedBlocks.toSeq)
          new InterruptibleIterator(context, cachedValues)
  ...
  //zengdan
  private def waitLockForPartition[T](id: RDDBlockId): Option[Iterator[T]] = {
    //loading.synchronized {
      if (!loading.contains(id)) {
        // If the partition is free, acquire its lock to compute its value
        None
        ...
        logInfo(s"Finished waiting for $id")
        blockManager.get(id).map(_.data.asInstanceOf[Iterator[T]])
      }
    //}
  }
