...
abstract class RDD[T: ClassTag](
  ...
  //zengdan
  var pipelinebreaker = false

  def toArr(a:Array[_]) = a.asInstanceOf[Array[T]]
  def toT(x: Any) = x.asInstanceOf[T]
  def linkArrayFunc(s: Partition) = {}
  def linkSingleFunc(s: Partition) = {}
  def setArrayfcs(s: Partition) = {
    //fcs = toArr _
    //curf = toT _
    arrayf.put(s, toArr _)
    //singlef.put(s, toT _)
  }
  def setSinglefcs(s: Partition) = {
    //fcs = toArr _
    //curf = toT _
    //arrayf.put(s, toArr _)
    singlef.put(s, toT _)
  }
  val arrayf = new HashMap[Partition, Array[_] => Array[T]]

  var fcs:Array[_] => Array[T] = toArr _
  //var curfcs: ArrayBuffer[_] => Array[T] = x => x.asInstanceOf[Array[T]]
  var curifcs: Iterator[_] => Iterator[T] = x => x.asInstanceOf[Iterator[T]]
  val singlef = new HashMap[Partition, Any => T]
  var curf: Any => T = toT _

  def getMaterializeIter(src:Iterator[_], s: Partition, arraymode: Boolean) = {
    val rdd = this
    if(!arraymode){
      new Iterator[T] {
        private val oldsrc = src
        private val func = singlef.get(s)

        override def hasNext = oldsrc.hasNext
        override def next() = func(oldsrc.next())//rdd.curf(oldsrc.next())
      }
    }else {
      new Iterator[T] {
        private val oldsrc = src
        ...
        private val func = arrayf.get(s)

        override def hasNext = {
          while ((curpos == -1 || curpos >= current.length) && oldsrc.hasNext) {
            current = func(Array(oldsrc.next())) //rdd.fcs(Array(oldsrc.next()))
            curpos = 0
          }
          curpos > -1 && curpos < current.length
        }
  ...
  def getParent: Option[RDD[_]] = None
  def getParentRDD(s: Partition):Option[RDD[_]] = None
  def getParentPartition(s: Partition):Partition = s
  ...
  //zengdan
  final def exists(split: Partition, context: TaskContext): Option[Iterator[T]] = {
    if (storageLevel != StorageLevel.NONE) {
      SparkEnv.get.cacheManager.get(this, split, context, storageLevel)
    } else {
      None
    }
  }
  ...
  def mapPartitions[U: ClassTag](
      ...
      pipeline: Boolean = false): RDD[U] = {
    val func = (context: TaskContext, index: Int, iter: Iterator[T]) => f(iter)
    //new MapPartitionsRDD(this, sc.clean(func), preservesPartitioning)

    new MapPartitionsWithNoneRDD(this, sc.clean(f), preservesPartitioning, pipeline)
  }
  ...
  def collect(): Array[T] = {
    //zengdan
    var start = System.nanoTime() //zengdan
    val results = sc.runJob(this, (iter: Iterator[T]) => iter.toArray)
    println("====Time to run job is %f s".format((System.nanoTime - start) / 1e9)); //zengdan
    start = System.nanoTime() //zengdan
    val r = Array.concat(results: _*)
    println("====Time to concat result is %f s".format((System.nanoTime - start) / 1e9)); //zengdan
    r
  }
  ...
  def count(): Long = {
    ...
    val rst = sc.runJob(this, Utils.getIteratorSize _).sum
    //zengdan
    println("run count")
    rst
  }
